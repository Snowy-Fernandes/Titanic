...
.block-container {{
    ...
    padding: 1.25rem 1.5rem 130px !important;
...
.msg-meta {{ display: flex; align-items: center; gap: 0.3rem; margin-top: 0.3rem; }}
.msg-time {{ font-size: 0.6rem; color: {TEXT_DIM}; font-family: 'JetBrains Mono', monospace; }}
...
.empty-state h3 {{ font-size: 1rem; font-weight: 600; color: {TEXT}; margin: 0 0 0.5rem; letter-spacing: -0.02em; }}
.empty-state p {{ font-size: 0.78rem; max-width: 320px; margin: 0 auto; line-height: 1.6; opacity: 0.7; }}
...
.stChatInput > div:focus-within {{
    ...
    box-shadow: 0 0 0 3px {GLOW}, 0 2px 12px rgba(0,0,0,0.10) !important;
}}
...
.stChatInput button[data-testid="stChatInputSubmitButton"] {{
    ...
    box-shadow: {SEND_SHADOW} !important;
    transition: transform 0.15s ease, box-shadow 0.15s ease !important;
    margin: 0.3rem 0.35rem 0.3rem 0 !important;
    width: 36px !important;
    height: 36px !important;
}}
.stChatInput button[data-testid="stChatInputSubmitButton"]:hover {{
    transform: scale(1.08) !important;
    box-shadow: 0 0 36px rgba(59,108,255,0.65) !important;
}}
.stChatInput button[data-testid="stChatInputSubmitButton"]:active {{
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f"box-shadow:0 4px 12px {GLOW};'>🚢</div>"
        f"<span style='font-size:0.92rem;font-weight:700;letter-spacing:-0.02em;'>Titanic Chatbot</span></div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<p style='font-size:0.68rem;color:{TEXT_DIM} !important;"
        f"margin:0 0 1rem 0;padding-left:2px;'>LangChain · HuggingFace</p>",
        unsafe_allow_html=True,
    )
